...
| REQ-18 | Tras un login o registro exitoso, el frontend MUST redirigir: `professor` → `/dashboard`; `student` → `/portal-alumno`. |
...
- WHEN el frontend recibe el token y decodifica el rol
- THEN el usuario es redirigido a `/portal-alumno` (shell del alumno con bienvenida personalizada)

#### Scenario: Shell del alumno muestra bienvenida

- GIVEN un alumno está autenticado y en `/portal-alumno`
...
- WHEN navega manualmente a `/portal-alumno`
...
- THEN `RoleRoute` lo redirige a `/portal-alumno`
...
- WHEN navega directamente a `/dashboard` o `/portal-alumno`
...
- [ ] El login redirige a `/dashboard` si `role === "professor"` y a `/portal-alumno` si `role === "student"`.
